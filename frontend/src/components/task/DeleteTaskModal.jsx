...
const DeleteTaskModal = ({ isOpen, onClose, task, onSuccess }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || error.response?.data?.msg || 'Failed to delete task');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-md">
      <div className="bg-surface-container border border-outline-variant rounded-xl p-lg max-w-md w-full space-y-lg shadow-2xl relative animate-in fade-in zoom-in-95 duration-200 text-on-background">
        ...
        <div className="text-left space-y-2">
          ...
          <h3 className="text-headline-md font-bold text-on-surface">Delete Task #{task.id}</h3>
          <p className="text-body-md text-on-surface-variant">
            Are you sure you want to delete <span className="font-semibold text-on-surface">"{task.title}"</span>? This action cannot be undone.
          </p>
        </div>
