...
const CreateTicketModal = ({ isOpen, onClose, projectId, projectName, canCreateTicket = true, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!canCreateTicket) {
      toast.error('Only workspace owner, admin, or project admin can create tickets');
      return;
    }
    if (!form.title.trim()) {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || error.response?.data?.msg || 'Failed to create ticket');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-md">
      <div className="bg-surface-container border border-outline-variant rounded-xl p-lg max-w-md w-full space-y-lg shadow-2xl relative animate-in fade-in zoom-in-95 duration-200 text-on-background max-h-[90vh] overflow-y-auto custom-scrollbar">
        ...
        <div className="text-left">
          <h3 className="text-headline-md font-bold text-on-surface">Create New Ticket</h3>
          <p className="text-body-md text-on-surface-variant mt-xs">
            Add a ticket to {projectName || 'this project'}.
          </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-md text-left">
          <div className="space-y-xs">
            <label className="font-label-sm text-on-surface-variant block ml-xs font-semibold" htmlFor="ticketTitle">
              Title *
            </label>
            <input
          ...
          <div className="space-y-xs">
            <label className="font-label-sm text-on-surface-variant block ml-xs font-semibold" htmlFor="ticketDescription">
              Description
            </label>
            <textarea
          ...
          <div className="grid grid-cols-2 gap-md">
            <div className="space-y-xs">
              <label className="font-label-sm text-on-surface-variant block ml-xs font-semibold" htmlFor="ticketType">
                Type *
              </label>
              <select
            ...
            <div className="space-y-xs">
              <label className="font-label-sm text-on-surface-variant block ml-xs font-semibold" htmlFor="ticketPriority">
                Priority *
              </label>
              <select
                className="w-full h-12 px-4 rounded-lg bg-surface-container-low border border-outline-variant text-on-surface font-body-md focus:ring-2 focus:ring-primary focus:outline-none cursor-pointer"
          ...
          <div className="grid grid-cols-2 gap-md">
            <div className="space-y-xs">
              <label className="font-label-sm text-on-surface-variant block ml-xs font-semibold" htmlFor="ticketStatus">
                Status *
              </label>
              <select
                className="w-full h-12 px-4 rounded-lg bg-surface-container-low border border-outline-variant text-on-surface font-body-md focus:ring-2 focus:ring-primary focus:outline-none cursor-pointer"
            ...
            <div className="space-y-xs">
              <label className="font-label-sm text-on-surface-variant block ml-xs font-semibold" htmlFor="ticketDueDate">
                Due Date
              </label>
              <input
                className="w-full h-12 px-4 rounded-lg bg-surface-container-low border border-outline-variant text-on-surface font-body-md focus:ring-2 focus:ring-primary focus:outline-none cursor-pointer"
